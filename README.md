...
* dev-perl/Minilla - CPAN module authoring tool, recently suggested. The package is being patched (according to my Minilla fork) for making release.hooks run not at 'minil release' only, but at 'minil dist' too. So far, the package is being installed in minimal configuration (without "recommended" dependencies). Absent in ::gentoo.  
